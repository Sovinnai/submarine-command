...
COMMAND_CONTRACT = {
    ...
    "concurrency": {
        ...
        "towed_array": "stream_array needs 15 productive minutes at 8 knots or less; recover_array needs 10. Only minutes actually spent at or below that speed count, and progress is kept across interrupted windows. Hull and flank still listen during those activities. While the array is streaming, streamed or recovering, ordered speed may not exceed the published limit for that state. A turn, or the five minutes after one, marks the towed receiver unstable; the engine does not calculate cable shape or layback.",
...
def observe_contact(state, actor, dice, mode="passive", opening=False, array_spec=None):
    ...
    unstable = bool(
        ...
        and t < towed.get("unstable_until", 0)
    )
...
def new_world(seed):
    ...
    state["bearing_bias"] = dice.between("initial:bearing-bias", -1.0, 1.0)
    state["array_bearing_bias"] = {
        spec.identifier: dice.between(f"initial:bearing-bias:{spec.identifier}", -2.0, 2.0)
...
def public_sonar_receivers(state):
    ...
    charted = state["environment"]["measured"]["water_depth_feet"]
    towed = state.get("towed") or {}
    unstable = state["t"] < towed.get("unstable_until", 0)
    rows = []
...
def _update_towed_stability(state, course_before):
    ...
    if turning:
        ...
        if record["unstable_until"] < until:
            was_stable = state["t"] >= record["unstable_until"]
        ...
        return
    if not turning and record["unstable_until"] and state["t"] >= record["unstable_until"]:
        report(
            state,
            "Sonar",
...
def apply_towed_array(state, order, achieved):
    ...
    if activity == "stream_array":
        ...
        return
    progress = TICK if record["deployment"] != arrays.RECOVERING else record["progress_minutes"] + TICK
