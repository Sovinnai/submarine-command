...
COMMAND_CONTRACT = {
    ...
    "concurrency": {
        ...
        "operating_mode": "A commanded operating mode takes effect once actual speed and depth satisfy that mode's published limits; until then the previous mode remains in force. The mode's published speed limit is its only resolved effect on own-ship radiated noise in this rules version. Opposing detection uses own-ship speed, the shared transmission-loss model, receiver depth and active transmission; it does not yet read the plant mode.",
    },
    "order_fields": {
