...
def _listen_key(elapsed, mode):
    return f"{elapsed}:{mode}"


def observe_contact(state, actor, dice, mode="passive"):
    ...
    # A second request for the same look returns the stored result. It does
    # not draw, add lines, or create another evidence contribution.
    if track is not None and _listen_key(t, mode) in track.get("listens", []):
        return
    ...
    if mode == "active":
        active_excess, _loss = reception_signal_excess(state, actor, own, mode, extra_di)
        active_excess += window_db
        active_detected = dice.u(
            f"detect:{actor['id']}:{t}:{mode}"
        ) < acoustics.detection_probability(active_excess)
    # Draw the bearing error even when this look keeps no observation, so a
    # louder or quieter geometry does not change how many event draws exist.
    measured_bearing = (
        bearing(own, actor) + state["bearing_bias"]
        + dice.between(f"bearing:{actor['id']}:{t}:{mode}", -4, 4)
    ) % 360
    detected = measurement["any_detected"] or active_detected
    if track is not None:
        track.setdefault("listens", []).append(_listen_key(t, mode))
    if not detected:
        return
    new = track is None
    if new:
    ...
    if mode == "active" and active_detected:
...
def new_world(seed):
    ...
    # creates a track. Background traffic is scanned too, so a loud contact
    # is not withheld until the first command window.
    for actor in state["actors"]:
        observe_contact(state, actor, dice)
    return state
...
def mast_observations(state, dice):
    """Run one mast visual cycle. Returns how many visual reports it added."""
    seen = 0
    for actor in state["actors"]:
...
def tick_world(state, dice, order, first_tick):
    ...
    if order["activity"] == "mast" and achieved["mast_minutes"] < TICK:
        ...
        if mast_observations(state, dice) == 0:
            report(
                state, "Scope",
                "Mast cycle complete. No visual contact this look.",
                "routine",
            )
